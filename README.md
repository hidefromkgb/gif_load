...
incapable of that: the file must be resident before the first pixel, making the
read additive.

Warm, `gif_load` leads every column: 1.3x–1.7x against `giflib`, and 1.4x–2.5x
against `stb_image` (but reading GIFs once again while cached is not common in
day-to-day use). Cold, it's 1.3x–1.5x for all 3 shapes (closest thing to a real
GIF) and 0.83x–0.85x with the 3 noisy ones — both figures against `giflib`, as
`stb_image` is slower than `gif_load` even here.
...
Cold `read()` wins on readahead. Might be worth considering some prefetching?
